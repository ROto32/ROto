            ...
            allServerElements.forEach(function(serverElement) {
                ...
                allElements.forEach(function(element) {
                    if (!isExcludedButton(element) && element.textContent && element.textContent.toLowerCase().includes('share') && 
                        !element.classList.contains('roto-copy-join-link') &&
                        !element.classList.contains('roto-vip-invite-link') &&
                        !element.classList.contains('roto-vip-new-invite-link')) {
                        element.remove();
                    }
                });
            });
...
function processServerElements(selector, showFullId, inviteEnabled, checkPrivate = false) {
    ...
    for (var i = 0; i < serverElements.length; i++) {
        ...
        var serverId = server.getAttribute('data-roto-serverid');
        
        var vipServerId = server.getAttribute('data-roto-vipserverid');
        ...
        if (serverId && serverId.length > 0) {
            if (showFullId) {
                ...
                if (serverIDTextElement) {
                    ...
                    if (displayedServerId !== serverId) {
                        ...
                        server.setAttribute('data-roto-serverid-displayed', serverId);
                        
                        var existingCopyButton = server.querySelector('.roto-copy-join-link');
                        if (existingCopyButton) {
                            existingCopyButton.remove();
                        }
                        
                        var privateLabel = server.querySelector('.roto-private-server-label');
                        if (privateLabel) {
                            privateLabel.remove();
                        }
            ...
            if (checkPrivate) {
                server.setAttribute('data-roto-invite-enabled', inviteEnabled);
...
function checkIfServerIsPrivate(server, serverId) {
    if (server.hasAttribute('data-roto-private-checked')) {
        if (server.getAttribute('data-roto-is-private') === 'false') {
    ...
    server.setAttribute('data-roto-private-checked', 'true');
    ...
    .then(data => {
        if (data.status === 12 || 
            ...
            server.setAttribute('data-roto-is-private', 'true');
            
            if (!server.querySelector('.roto-private-server-label')) {
            ...
            server.setAttribute('data-roto-is-private', 'false');
            
            chrome.storage.local.get(['enableFriendservers', 'showfullserveridEnabled'], function(result) {
                var enableFriendservers = result.enableFriendservers !== false;
                var inviteEnabled = server.getAttribute('data-roto-invite-enabled') !== 'false';
                var showFullId = result.showfullserveridEnabled !== false;
...
function createAndAddPrivateServerLabel(server) {
    ...
    label.className = 'text-info rbx-game-status rbx-friends-game-server-status text-overflow roto-private-server-label';
...
function createAndAddCopyJoinLinkButton(server, serverId, serverIDTextElement = null) {
    var existingButton = server.querySelector('.roto-copy-join-link');
    if (existingButton) {
    ...
    copyButton.className = 'btn-full-width btn-control-xs rbx-public-game-server-join btn-primary-md btn-min-width roto-copy-join-link';
    copyButton.textContent = 'Copy Join Link';
    copyButton.style.marginTop = '5px';
    copyButton.style.width = '100%';
    copyButton.setAttribute('data-serverid', serverId);
    copyButton.setAttribute('data-roto-button-id', 'copy-' + serverId);
    ...
    if (server.classList.contains('rbx-friends-game-server-item')) {
        ...
        if (detailsElement) {
            ...
            if (serverIDTextElement && serverIDTextElement.closest('.rbx-friends-game-server-details.game-server-details')) {
                ...
                if (!container.classList.contains('roto-server-id-container')) {
                    var newContainer = document.createElement('div');
                    newContainer.className = 'roto-server-id-container';
                ...
                mutation.addedNodes.forEach(function(node) {
                    if (node.nodeType === 1) {
                        ...
                        if (node.classList && node.classList.contains('rbx-private-game-server-item')) {
                            var vipServerId = node.getAttribute('data-roto-vipserverid');
                            if (vipServerId) {
...
function addVipServerInviteButton(server, vipServerId) {
    ...
    const existingButtonContainer = server.querySelector('.roto-vip-buttons-container[data-vipserverid="' + vipServerId + '"]');
    if (existingButtonContainer) { if (cachedStatus !== 'success') { vipStatusCache[vipServerId] = 'success'; } return; }
    if (cachedStatus === 'no-link' || cachedStatus === 'error') { let container = server.querySelector('.roto-vip-buttons-container'); if (container) container.remove(); return; }
    let oldContainer = server.querySelector('.roto-vip-buttons-container');
    if (oldContainer) { oldContainer.remove(); }
    server.querySelectorAll('.roto-vip-invite-link, .roto-vip-new-invite-link').forEach(btn => btn.remove());
            ...
            if (inviteLinkString) {
                chrome.storage.local.get(['privateserverlink'], function(result) {
                    if (result.privateserverlink === true) {                        vipStatusCache[vipServerId] = 'success';
                        ...
                        buttonContainer.className = 'roto-vip-buttons-container';
                        ...
                        copyButton.type = 'button';
                        copyButton.className = 'btn-full-width btn-control-xs rbx-public-game-server-join btn-primary-md btn-min-width roto-vip-invite-link';
                        ...
                        generateButton.className = 'btn-full-width btn-control-xs rbx-public-game-server-join btn-secondary-md btn-min-width roto-vip-new-invite-link';
                        ...
                        let container = server.querySelector('.roto-vip-buttons-container');
