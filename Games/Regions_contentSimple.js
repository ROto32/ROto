...
const RATE_LIMIT_CHECK_INTERVAL_MS = 1000;
    ...
    if (regionDropdownButton) {
        let statusIndicator = document.getElementById('rotoStatusIndicator');
        if (!statusIndicator) {
             statusIndicator = document.createElement('span');
             statusIndicator.id = 'rotoStatusIndicator';
    ...
    rateLimitCheckInterval = setInterval(async () => {
        ...
        try {
            ...
            if (response.status !== 429) {
                ...
                const regionListContainer = document.getElementById('roto-region-list-container');
             ...
             async function fetchThumbnailsBatch(tokens) {
                ...
                for (let i = 0; i < tokens.length; i += batchSize) {
                    ...
                    const fetchPromise = fetch(baseUrl, {
                      ...
                      }).catch(error => console.error('ROto: Thumbnail batch error:', error));
...
async function updatePopup(retries = 5) {
    ...
    refreshButton.onclick = async (e) => {
        ...
        if (!isRefreshing) {
            ...
            const listContainer = document.getElementById('roto-region-list-container');
            if(listContainer) { 
    ...
    regionListContainer.id = 'roto-region-list-container';
...
function injectRegionDropdownStyles() {
    if (document.getElementById('roto-region-dropdown-styles')) return;
    
    const styleElement = document.createElement('style');
    styleElement.id = 'roto-region-dropdown-styles';
    styleElement.textContent = `
        ...
        #regionDropdown.has-blur #roto-region-list-container {
            position: relative;
        }
        #regionDropdown.has-blur #roto-region-list-container > div[style*="text-transform: uppercase"] {
        ...
        #roto-region-list-container::-webkit-scrollbar {
            width: 6px;
            position: absolute;
            right: 2px;
        }
        
        #roto-region-list-container::-webkit-scrollbar-track {
            background: transparent;
        }
        
        .dark #roto-region-list-container::-webkit-scrollbar-thumb {
            background-color: rgba(255, 255, 255, 0.2);
            border-radius: 10px;
        }
        
        .light #roto-region-list-container::-webkit-scrollbar-thumb {
            background-color: rgba(0, 0, 0, 0.15);
            border-radius: 10px;
        }
        
        .dark #roto-region-list-container::-webkit-scrollbar-thumb:hover {
            background-color: rgba(255, 255, 255, 0.3);
        }
        
        .light #roto-region-list-container::-webkit-scrollbar-thumb:hover {
            background-color: rgba(0, 0, 0, 0.25);
        }
        
        /* Ensure scrollbar doesn't take space */
        #roto-region-list-container {
            margin-right: 0;
        ...
        #roto-region-list-container::-webkit-scrollbar-thumb {
            transition: opacity 0.3s;
        }
        
        #roto-region-list-container:not(:hover)::-webkit-scrollbar-thumb {
            opacity: 0.5;
        }
        
        /* Ensure items in the list don't get cut by scrollbar */
        #roto-region-list-container > div {
            margin-right: 0;
            position: relative;
        }
    `;
    ...
    if (unknownServerCount > 0) {
        ...
        listContainer.insertBefore(listItem, unknownHeader.nextSibling);
            ...
            async function showRegionServerListOverlay(region) {
                const existingOverlay = document.getElementById('roto-server-list-overlay');
                ...
                try {
                    ...
                    modalOverlay.id = 'roto-server-list-overlay';
                    ...
                    modalContent.id = 'roto-server-list-content';
                    const isDarkMode = currentTheme === 'dark';
                    ...
                    const closeModal = () => {
                            const overlay = document.getElementById('roto-server-list-overlay');
                            if (overlay) {
                            overlay.style.opacity = '0';
                            overlay.querySelector('#roto-server-list-content').style.transform = 'scale(0.95)';
                            setTimeout(() => {
                                overlay.remove();
                                body.style.overflow = originalBodyOverflow;
                                const listArea = document.getElementById('roto-server-list-content-area');
                    ...
                    const closeButton = document.createElement('button');
                        closeButton.id = 'roto-overlay-close-button';
                    ...
                    serverListArea.id = 'roto-server-list-content-area';
                    serverListArea.style.cssText = `flex-grow: 1; overflow-y: auto; padding: 10px 20px;`;
                    const serverList = document.createElement('div');
                    serverList.id = 'roto-actual-server-list';
            ...
            async function renderFullServerList() {
                const listElement = document.getElementById('roto-actual-server-list');
            ...
            function createServerEntryElement(server, thumbnailUrls, isDarkMode) {
                ...
                if (!joinButton.disabled) {
                    joinButton.onclick = () => {
                        ...
                        const overlay = document.getElementById('roto-server-list-overlay');
                        if (overlay) {
                            const actualCloseButton = overlay.querySelector('#roto-overlay-close-button');
