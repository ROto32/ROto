...
async function fetchDataFromAPI(retryCount = 0, maxRetries = 1, retryDelay = 100) {
    // You are allowed to use this API for personal projects only which is limited to open source projects on GitHub, they must be free and you must credit the ROto repo.
    // You are not allowed to use the API for projects on the chrome web store or any other extension store. If you want to use the API for a website dm be on discord: oto and we can figure something out.
    // If you want to use the API for something thats specifically said isnt allowed or you might be unsure if its allowed, please dm me on discord: oto, Ill be happy to check out your stuff and maybe allow you to use it for your project.
    const apiUrl = 'https://catalog.roto.com/items';
